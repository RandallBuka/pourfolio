...
import { isAfterIngredientImageAnchor } from './ingredientImageAnchor'
import { isProfessionalIngredientImageUrl } from './ingredientImageQuality'
...
function isAllowedImageUrl(ingredient: Ingredient, url: string): boolean {
  ...
  // User-curated entries at/before black olives: show stored URL as-is.
  if (!isAfterIngredientImageAnchor(ingredient.id)) return true

  return isProfessionalIngredientImageUrl(url, ingredient.name)
}
