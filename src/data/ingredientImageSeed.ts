...
function loadAllIngredients(): Array<Pick<Ingredient, 'id' | 'name' | 'genericName'>> {
  const byId = new Map<string, Pick<Ingredient, 'id' | 'name' | 'genericName'>>()
  for (const ing of CORE_SEED_INGREDIENTS) {
    byId.set(ing.id, { id: ing.id, name: ing.name, genericName: ing.genericName })
  }
  try {
