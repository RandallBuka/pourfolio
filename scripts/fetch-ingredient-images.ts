 ...
 * Replace bad URLs: npm run refresh-ingredient-images
...
async function main() {
  ...
  const work = INGREDIENT_IMAGE_SEED.filter((ing) => {
    ...
    const url = map[ing.id] ?? ''

    if (!onlyEmpty) return true

    if (refreshInvalid && url && !isProfessionalIngredientImageUrl(url, ing.name)) return true
  ...
  const mode = !onlyEmpty ? ' (all)' : refreshInvalid ? ' (empty + invalid)' : ' (empty only)'

  console.log(`Processing ${work.length} ingredients after anchor${mode} (concurrency ${concurrency})`)
  ...
  for (let batchStart = startIndex; batchStart < work.length; batchStart += concurrency) {
    ...
    for (const { ing, url } of results) {
      ...
      if (url && isProfessionalIngredientImageUrl(url, ing.name)) {
